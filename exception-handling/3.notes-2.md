...
| Parent Class          | `Exception`                                  | `LinkageError`                                         |
| Happens When          | JVM cannot find class while manually loading | Class available at compile time but missing at runtime |
| Loading Type          | Explicit loading                             | Implicit loading                                       |
| Common Cause          | Missing class in classpath                   | Missing `.class` file at runtime                       |
| Mostly Used With      | `Class.forName()`                            | Object creation / method call                          |
| Compile Time Presence | Not necessary                                | Present                                                |
| Runtime Presence      | Missing                                      | Missing                                                |
| Handling              | Must handle using `try-catch`                | Usually not handled                                    |
| Nature                | Runtime loading issue                        | Deployment / runtime linkage issue                     |
| Recovery              | Possible by fixing classpath                 | Usually requires deployment fix                        |
| Example               | `Class.forName("Test")`                      | `new Helper()`                                         |
| Common Scenario       | JDBC Driver loading                          | Missing dependency JAR                                 |
| JVM Behavior          | Programmer asks JVM to load class            | JVM automatically tries to load class                  |
...
| Situation                     | Result                   |
| ----------------------------- | ------------------------ |
| You manually load class       | `ClassNotFoundException` |
| JVM automatically loads class | `NoClassDefFoundError`   |

---

### Memory Trick

| Exception/Error        | Memory Line                                |
| ---------------------- | ------------------------------------------ |
| ClassNotFoundException | "Class not found while searching manually" |
| NoClassDefFoundError   | "Class existed before, but missing now"    |
